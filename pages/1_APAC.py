...
if not df.empty:
    ...
    with col1:
        ...
        selected_countries = st.multiselect("🏳️ Countries", options=available_countries, default=available_countries, key="countries_apac")
    ...
    with col2:
        ...
        selected_exchanges = st.multiselect("🏢 Exchanges", options=available_exchanges, default=available_exchanges, key="exchanges_apac")

    with col3:
        available_sectors = sorted([s for s in df['sector'].dropna().unique().tolist()])
        selected_sectors = st.multiselect("🏭 Sectors", options=available_sectors, default=available_sectors, key="sectors_apac")
