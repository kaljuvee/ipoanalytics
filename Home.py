...
if not df.empty:
    ...
    # Display three separate regional treemaps
    regions_to_display = ['EMEA', 'Americas', 'APAC']
    ...
    if not region_df.empty:
        # Display region header with emoji
        region_emoji = {'EMEA': '🌍', 'Americas': '🌎', 'APAC': '🌏'}
        st.markdown(f"## {region_emoji.get(region, '🌐')} {region} Region")
        
        # Display key metrics for this region
        col1, col2, col3, col4 = st.columns(4)
        
        with col1:
            st.metric("Total IPOs", len(region_df))
        
        with col2:
            avg_performance = region_df['price_change_since_ipo'].mean()
            st.metric("Avg Performance", format_percentage(avg_performance))
        
        with col3:
            total_market_cap = region_df['market_cap'].sum()
            st.metric("Total Market Cap", format_market_cap(total_market_cap))
        
        with col4:
            if len(region_df) > 0:
                best_performer = region_df.loc[region_df['price_change_since_ipo'].idxmax()]
                st.metric("Best Performer", f"{best_performer['ticker']} ({format_percentage(best_performer['price_change_since_ipo'])})")
        
        # Create treemap for this region
        if len(region_df) > 0:
            # Prepare treemap data
            treemap_df = region_df.copy()
            treemap_df = treemap_df.dropna(subset=['country', 'sector', 'ticker'])
            
            # Create hierarchical path for treemap
            treemap_df['path'] = (
            ...
            fig = px.treemap(
                ...
                title=f"{region} IPO Performance - {selected_timeframe}",
            ...
            # Use all data for country overview and other sections
            filtered_df = df
    
            # Display overall key metrics
            st.markdown("## 📊 Global IPO Overview")
            col1, col2, col3, col4 = st.columns(4)
